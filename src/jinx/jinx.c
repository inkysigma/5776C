...
// Port over which all serial communication will occur. STDIN == STDOUT, so
// either can be used
...
int readLine(JINX *inStr) {
  ...
  // Get character from serial. If first character is terminator, quit
  // immediately
  ...
  // This means that if only a terminator is read, string is only a null
  // terminator
