...
void twenty() {
	...
	moveMobileGoal(0);
	/**
	// attempt to retrive the cone
	moveLift(-100);
	delay(100);
	moveMobileGoal(0);
	while (!within(SensorValue[lift], 1320, 100)) { moveLift(1320 - SensorValue[lift]); moveVertibar(538 - SensorValue[vertibar]); }
	while (!within(SensorValue[vertibar], 538, 100)) { moveVertibar(-80); }
	moveVertibar(0);
	moveDrive(127, 127);
	delay(1500);
	moveDrive(0, 0);
	...
	delay(100);
	openClaw(30);
	while (!within(SensorValue[vertibar], 2900, 40)) { moveVertibar(2900 - SensorValue[vertibar]); }
	moveVertibar(0);
	openClaw(-40);
	moveLift(100);**/
