...
SUBDIRS=src src/debug src/pid src/ops src/util
...
pclean:
	-rm -f $(BINDIR)/opcontrol.h $(BINDIR)/init.h $(BINDIR)/auto.o
...
$(OUT): $(SUBDIRS) $(ASMOBJ) $(COBJ) $(CPPOBJ)
	@echo LN $(BINDIR)/*.o $(wildcard $(BINDIR)/**/*.o) $(LIBRARIES) to $@
	@$(CC) $(LDFLAGS) $(BINDIR)/*.o $(wildcard $(BINDIR)/**/*.o) $(LIBRARIES) -o $@
